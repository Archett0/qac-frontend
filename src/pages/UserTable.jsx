...
const UserTable = () => {
    ...
    useEffect(() => {
        const getToken = async () => {
            ...
            if (token) {
                try {
                    setCurrentUserId(user['https://your-domain.com/uuid']);
        ...
        getToken();
    }, [currentUserId, getAccessTokenSilently, user]);

    useEffect(() => {
        const getUserProfile = async () => {
            try {
                const token = await getAccessTokenSilently();
                const userProfile = await fetchUserProfile(currentUserId, token);
        ...
        getUsers();
    }, [currentUserId, getAccessTokenSilently]);

    const handleSearch = async () => {
        setLoading(true);
        try {
            const token = await getAccessTokenSilently();
            const data = await searchUsers(searchKeyword, token);
            setUsers(data);
        } catch (err) {
