...
const MyAccount = () => {
    ...
    useEffect(() => {
        const getUserProfile = async () => {
            if (isAuthenticated) {
                try {
                    ...
                    const userProfile = await fetchUserProfile(user['https://your-domain.com/uuid'], token);
        ...
        const getQuestions = async () => {
            try {
                const token = await getAccessTokenSilently();
                const data = await fetchQuestionsByOwnerId(user['https://your-domain.com/uuid'], token);
                setQuestions(data);
            } catch (err) {
    ...
    return (
        <Box sx={{maxWidth: '800px', margin: '0 auto', padding: '20px'}}>
            <Paper elevation={3} sx={{padding: '20px', marginBottom: '20px', textAlign: 'center'}}>
                <Avatar alt="User Avatar" src="https://i.pravatar.cc/150?img=10"
                    ...
                    >
                        ...
                        <Box sx={{flex: 1}}>
                            ...
                            <Box sx={{display: 'flex', justifyContent: 'flex-end', alignItems: 'center', gap: '8px'}}>
                                <IconButton component={Link} to={`/questions/show/${question.id}`} color="primary"
                                            size="small">
                                    <VisibilityIcon/>
                                </IconButton>
                                <IconButton component={Link} to={`/questions/update/${question.id}`} color="secondary"
                                            size="small">
                                    <EditIcon/>
                                </IconButton>
